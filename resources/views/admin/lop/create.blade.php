...
@section('page_title', 'Thêm Lớp Hành Chính Mới')
    ...
    <div class="card card-modern col-lg-9 col-xl-8 mx-auto shadow-sm">
        <div class="card-modern-header">
            <div class="title-group">
                ...
                <div>
                    <h1 class="main-title">Thêm Lớp Hành Chính Mới</h1>
                    <div class="sub-title">Tạo mới lớp sinh viên, gán ngành học và khóa học/học kỳ</div>
                </div>
            </div>
            ...
            <form action="{{ route('lop.store') }}" method="POST">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="TenLop" value="{{ old('TenLop') }}" class="form-control form-control-modern" required placeholder="Ví dụ: 12DHTH01 hoặc 21DTH01">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="MaNganh" class="form-select form-select-modern" required>
                            ...
                            @foreach($nganhs as $nganh)
                                <option value="{{ $nganh->MaNganh }}" {{ old('MaNganh') == $nganh->MaNganh ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <label class="form-label-modern">
                            <i class="fa-solid fa-calendar-days me-1 text-cyan"></i> Chọn Học Kỳ Khai Giảng <span class="text-danger">*</span>
                        </label>
                        <select name="MaHocKy" class="form-select form-select-modern" onchange="document.getElementById('KhoaHocInput').value = this.options[this.selectedIndex].getAttribute('data-namhoc') || this.value;">
                            <option value="">-- Chọn học kỳ mở lớp --</option>
                            @foreach($hocKies as $hk)
                                <option value="{{ $hk->MaHocKy }}" data-namhoc="{{ $hk->NamHoc }}" {{ old('MaHocKy') == $hk->MaHocKy ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" id="KhoaHocInput" name="KhoaHoc" value="{{ old('KhoaHoc', '12') }}" class="form-control form-control-modern" required placeholder="Ví dụ: 12 hoặc 2021-2025">
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-3 mt-5 pt-3 border-top">
                    ...
                    <button type="submit" class="btn btn-cyan btn-sm rounded-pill px-4 py-2">
                        <i class="fa-solid fa-plus me-1"></i> Lưu Lớp Mới
                    </button>
                </div>
            </form>
        </div>
